...
func (aiParser AIParser) ParseInvoice(ctx echo.Context) error {
	...
	for key, values := range ctx.Request().Form { // range over map
		for _, value := range values { // range over []string
			if key == "authContext" {
				err = json.Unmarshal([]byte(value), &req.AuthContext)
				if err != nil {
					fmt.Println(err)
				}
			...
			if key == "bookingResponse" {
				err = json.Unmarshal([]byte(value), &req.BookingResponse)
				if err != nil {
					fmt.Println(err)
				}
				continue
			...
			if key == "paymentMethod" {
				req.PaymentMethod = value
				continue
			}
			if key == "paymentTerm" {
				req.PaymentTerm = value
				continue
			}
			if key == "accountingPlatform" {
				req.AccountingPlatform = value
				continue
			}
			if key == "includeShipping" {
				if value == "true" {
					req.IncludeShipping = true
				} else {
					req.IncludeShipping = false
				}
				continue
			}
		}
