...
func TestLogin(t *testing.T) {
	...
	serv := services.New(config)
	client := &http.Client{}
	encodedBody := fmt.Sprintf("code=%s&grant_type=authorization_code&redirect_uri=%s", code, serv.CognitoClient.RedirectUrl)
	...
	req.Header.Set("Authorization", "Basic "+serv.CognitoClient.BaseAuth)
...
func TestRefreshtoken(t *testing.T) {
	...
	serv := services.New(config)
	client := &http.Client{}
	encodedBody := fmt.Sprintf("refresh_token=%s&grant_type=refresh_token&client_id=%s", refreshToken, serv.CognitoClient.CognitoClientID)
	var data = strings.NewReader(encodedBody)
	req, err := http.NewRequest("POST", "https://firstshipper-dev.auth.us-west-1.amazoncognito.com/oauth2/token", data)
	if err != nil {
		log.Fatal(err)
