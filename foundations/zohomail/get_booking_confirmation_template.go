...
func (email Email) getBookingConfirmationTempalte(data EmailData, bookingRes *v1.BookingResponse) string {
	...
	template = strings.ReplaceAll(template, "{SHIPPER_COMPANY_NAME}", bookingRes.QuoteRequest.Delivery.CompanyName)
	template = strings.ReplaceAll(template, "{SHIPPER_NAME}", bookingRes.QuoteRequest.Delivery.Contact.Name)
	template = strings.ReplaceAll(template, "{SHIPPER_ADDRESS1}", bookingRes.QuoteRequest.Delivery.Address.AddressLine1)
	template = strings.ReplaceAll(template, "{SHIPPER_CITY}", bookingRes.QuoteRequest.Delivery.Address.City)
	template = strings.ReplaceAll(template, "{SHIPPER_ZIPCODE}", bookingRes.QuoteRequest.Delivery.Address.ZipCode)
	template = strings.ReplaceAll(template, "{SHIPPER_STATE}", bookingRes.QuoteRequest.Delivery.Address.State)
